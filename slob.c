/*
 * lab 3: modified by Daniel Gonzalez
 *
...
long mem_claimed[MAX_COUNT];
long mem_free[MAX_COUNT];
int mem_pos = 0, mem_count = 0;
...
#define GET_SIZE(slob_units) (slob_units * SLOB_UNIT - SLOB_UNIT + 1)
 ...
 * lab 3: modified to return the best block of the page
 */
...
/*
 * lab 3: check_best_fit: gives the best fit of a page: 0 if prefect fit,
 * -1 if no fit, positive int if after allocation there is space left
 */
static int check_best_fit(struct slob_page *sp, size_t size, int align)
{
	slob_t *prev, *cur, *aligned = NULL;
	int delta = 0, units = SLOB_UNITS(size);
 ...
 * lab 3: modified to use check_best_fit to find the best page
 */
static void *slob_alloc(size_t size, gfp_t gfp, int align, int node)
{
	...
	slob_t *b = NULL;
	unsigned long flags;
	bool is_slob_small = false, stop_stat = false;
	long mem_aux = 0;
	...
	list_for_each_entry(sp, slob_list, list) {
		...
		current_fit = check_best_fit(sp, size, align);
		if (current_fit == 0) {
			best_page = sp;
	...
	if (!b) {
		...
		if (is_slob_small) {
			if (mem_count < MAX_COUNT)
				mem_count++;
			mem_free[mem_pos] = mem_aux;
			mem_claimed[mem_pos] = size;
			mem_pos = (mem_pos + 1) % MAX_COUNT;
		}
...
// returns an average of the memory claimed
...
// returns an average of the free memory
